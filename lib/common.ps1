...
function Get-LwgFrontmatterClass {
    <#
      The `lw-class` value declared by one agent file, or '' for "no
      information" - unreadable, no frontmatter, no key, or a value that is not
      one of the three.

      SCANNED, not parsed, and by IndexOf rather than by `-split` and `-match` -
      see the note above this section. The regex engine's first use in a fresh
      process costs more than this whole function does, for a job two string
      operations already do. The frontmatter is also only WALKED as far as the
      closing fence, so the agent's system prompt below it is never examined.

      [IO.File]::ReadAllText, deliberately, rather than a bounded FileStream
      read. The bounded form was written first and measured SLOWER: opening a
      FileStream, allocating a buffer and decoding it are four .NET first-uses on
      the Stop path, against one for ReadAllText - which Get-LwgPluginName has
      already warmed by the time any hook reaches here. What that gives up is the
      cap on file size, and the reason that is acceptable is that this file is an
      agent role file: Claude Code loads the whole of it into a subagent's system
      prompt, so any file large enough for this read to matter is already a
      problem somewhere that matters more.

      Returns '' on ANY failure - locked, deleted between the probe and here,
      unreadable. That is "no information", and the caller must treat it as such.
    #>
    param([string]$Path)

    $head = ''
    try { $head = [IO.File]::ReadAllText($Path) } catch { return '' }
    if ([string]::IsNullOrEmpty($head)) { return '' }

    $head = $head.TrimStart([char]0xFEFF)

    # No opening fence means no frontmatter, so there is nothing to read. Note
    # what this does NOT do: it does not go looking for the key elsewhere in the
    # file. A `lw-class` written in the prose is not metadata and the loader
    # would not honour it either.
    $pos = $head.IndexOf([char]10)
    $first = $(if ($pos -ge 0) { $head.Substring(0, $pos) } else { $head })
    if ($first.Trim() -ne '---') { return '' }
    if ($pos -lt 0) { return '' }
    $pos++

    while ($pos -lt $head.Length) {
        $e = $head.IndexOf([char]10, $pos)
        $line = $(if ($e -ge 0) { $head.Substring($pos, $e - $pos) } else { $head.Substring($pos) })
        $t = $line.Trim()

        if ($t -eq '---') { return '' }           # closing fence; the key is absent

        $c = $t.IndexOf(':')
        if ($c -gt 0 -and $t.Substring(0, $c).TrimEnd() -ieq 'lw-class') {
            $v = $t.Substring($c + 1).Trim().Trim('"', "'").ToLowerInvariant()
            if ($script:LwgAgentClasses -contains $v) { return $v }
            return ''                              # a typo is not a class
        }
