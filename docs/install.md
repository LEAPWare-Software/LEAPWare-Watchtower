...
| `gh` on `PATH` | Optional. **Four** things use it: `git_hygiene`'s open-PR check at turn end, and three evidence probes behind `/lw-watchtower:checklist` and `/lw-watchtower:sitrep` — the `kind: ci` rule that reads the newest CI conclusion, and the two `gh` rules in [`checklist.json`](../checklist.json). `module_config.git_hygiene.use_gh: false` switches off **all four**; with it off, the three evidence rows render `[?] UNVERIFIED` saying so, which is "not checked" and not a finding either way. |
| `gh` and **whose repository it is asked about** | Read this before installing on a machine whose `gh` is authenticated. Two of `checklist.json`'s forty rows name **this plugin's own repository** by path — a branch-protection object and a visibility flag — so running `/lw-watchtower:checklist` makes an authenticated request, **as you**, about a repository that is not yours. It is read-only and it is the only outbound traffic this plugin generates that is not about your own tree. `use_gh: false` stops it. |
