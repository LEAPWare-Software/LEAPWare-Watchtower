...
**The hooks are the part that does not survive.** All fifteen registrations in
...
- **Hook registrations in `settings.json`, on a standalone install only.** When Claude Code can
  ...
  file declares. Today that is **15 registrations across 8 events** (`SessionStart`, `PreToolUse`,
