...
| `tests/config_behaviour.ps1` | 56 cases against `bin/lwg-config.ps1`, the module switchboard's write path, which nothing in `tests/` had ever executed: the refusals it is built around, the two-phase preview, the surgical JSON edit and the exit-2 read-back. Like the toggle suite it closes with an invariant that no run moved a byte of the plugin root's `config.json`. |
