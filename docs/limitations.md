...
| `tests/stop_behaviour.ps1` | 132 cases, the helpers in process and the hooks in real child processes: pinned behaviours of `failure_capture`, `context_pressure`, `docs_coupling`, `git_hygiene` and `log_rotation`, including two supervisor bugs that had already shipped, and the redaction helper every module's error text passes through — asserted to keep no part of a credential pasted into a prompt out of a state file or an advisory. |
