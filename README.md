...
- **Eleven suites test behaviour, and a green run of all of them is a narrower claim than the
  ...
  advisory modules behind them; `tests/setup_merge.ps1` runs 185 against the installer's `statusline`
  ...
  against the two commands that write `config.override.json`; `tests/state_resolution.ps1` runs 32
  against the state-directory resolver; `tests/doctor_behaviour.ps1` runs 32 against the doctor's
  driven checks; `tests/subagent_scan.ps1` runs 8 against the `SubagentStart` fast path — the only
