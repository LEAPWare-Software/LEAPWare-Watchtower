...
<!-- THIRTEEN SUITES TEST BEHAVIOUR, each through a real pipe into a real child process:
     ...
     scoreboard behind /lw-watchtower:metrics; stack_mode covers the module that resolves one
     working discipline per session and points every worker at its ruleset.
...
```
<!-- If your fix is inside what one of the thirteen behavioural suites covers - any of the three
     ...
     discloses, the transcript indexer and the scoreboard, or the working-discipline injector - ADD THE CASE TO THAT SUITE and paste its per-case line from before and after.
